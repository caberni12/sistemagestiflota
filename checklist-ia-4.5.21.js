/* E-fleet Web 4.5.21 · R8.2.331 · NEXO IA RESILIENTE: 429/cuota no bloquea Check-in */
(() => {
  'use strict';
  ...
  function evidenceMarkup(){return `<section class="checklist-ia-card full" data-checklist-ia-card>
    ...
    <input type="hidden" name="SESION_IA_ID"><input type="hidden" name="FOTOS_VEHICULO"><input type="hidden" name="IA_RESULTADO_CODIFICADO"><input type="hidden" name="IA_TRANSCRIPCION"><input type="hidden" name="KILOMETRAJE_CONFIRMADO_USUARIO" data-ia-km-confirm-value><input type="hidden" name="ODOMETRO_INTENTOS" value="0" data-ia-km-attempts><input type="hidden" name="KILOMETRAJE_CONTINGENCIA_MANUAL" value="NO" data-ia-km-manual-flag><input type="hidden" name="OMITIR_IA_CONTINGENCIA" value="NO" data-ia-contingency><input type="hidden" name="MODO_SIN_IA" value="NO"><input type="hidden" name="MOTIVO_IA_CONTINGENCIA" value="">
  ...
  function enhance(form){
    ...
    form.dataset.checklistIaEnhanced='1';form.dataset.iaOdometerAttempts='0';form.dataset.iaKmManualAvailable='';
  ...
  function isAiUnavailable(error){
    ...
    return /GEMINI|NEXO.*(?:429|CUOTA|CREDITO|CRÉDITO|LIMITE|LÍMITE)|RESOURCE_EXHAUSTED|RATE.?LIMIT|TOO MANY REQUESTS|QUOTA|(?:^|[^0-9])429(?:[^0-9]|$)/i.test(text);
  }
  function activateAiContingency(form,photos,error){
    const reason=String(error?.message||error||'IA_NO_DISPONIBLE').slice(0,240);
    if(form.elements.FOTOS_VEHICULO)form.elements.FOTOS_VEHICULO.value=JSON.stringify(Array.isArray(photos)?photos:[]);
    if(form.elements.SESION_IA_ID)form.elements.SESION_IA_ID.value='';
    if(form.elements.OMITIR_IA_CONTINGENCIA)form.elements.OMITIR_IA_CONTINGENCIA.value='SI';
    if(form.elements.MODO_SIN_IA)form.elements.MODO_SIN_IA.value='SI';
    if(form.elements.MOTIVO_IA_CONTINGENCIA)form.elements.MOTIVO_IA_CONTINGENCIA.value=reason;
    const flag=form.querySelector('[data-ia-km-manual-flag]');if(flag)flag.value='SI';
    form.dataset.iaContingency='1';form.dataset.iaKmManualAvailable='1';
    const km=form.elements.KILOMETRAJE;if(km){km.readOnly=false;km.removeAttribute('readonly');km.inputMode='numeric';km.tabIndex=0;km.placeholder='Ingrese el KM real visible en la foto del odómetro';}
    const kmWrap=form.querySelector('[data-ia-km-confirm-wrap]'),kmConfirm=form.querySelector('[data-ia-km-confirm]'),kmText=form.querySelector('[data-ia-km-confirm-text]');
    if(kmWrap)kmWrap.hidden=false;if(kmConfirm){kmConfirm.checked=false;kmConfirm.disabled=false;}if(kmText)kmText.textContent='IA temporalmente no disponible. Escriba el kilometraje real visible en la foto y confírmelo manualmente.';
    const bulk=form.querySelector('[data-checkin-mark-all-ok]');if(bulk){bulk.disabled=false;bulk.title='Modo contingencia: complete los 18 puntos manualmente.';}
    setStatus(form,'warning','Continuar sin IA','Las 5 evidencias quedaron guardadas. Complete manualmente los 18 controles y el kilometraje; el Check-in no se bloqueará por cuota/crédito de IA.');
    setProcessProgress(form,72,'Modo revisión humana sin IA','NEXO IA no está disponible por cuota/servicio. Continúe con kilometraje + revisión humana 18/18.');
    toast('NEXO IA no disponible','No es una falla de Internet. Continúe el Check-in manualmente; las evidencias ya quedaron conservadas.','warning');
  }
  ...
  async function process(form,submitter){
    ...
    if(submitter){submitter.disabled=true;submitter.textContent=form.dataset.iaContingency==='1'?'Validando revisión humana…':(form.elements.SESION_IA_ID?.value?'Validando revisión humana…':'Analizando con NEXO IA…');}
    try{
      let sessionId=form.elements.SESION_IA_ID?.value||'',photos=safeJson(form.elements.FOTOS_VEHICULO?.value||'',[]),analysis=null;
      const contingency=form.dataset.iaContingency==='1'||String(form.elements.OMITIR_IA_CONTINGENCIA?.value||'').toUpperCase()==='SI';
      if(!sessionId&&!contingency){
        ...
        const intentoActual=Number(form.dataset.iaOdometerAttempts||0)+1;form.dataset.iaOdometerAttempts=String(intentoActual);const attemptsInput=form.querySelector('[data-ia-km-attempts]');if(attemptsInput)attemptsInput.value=String(intentoActual);
        try{analysis=await api('analizarChecklistIaBorrador',{VEHICULO_ID:form.elements.VEHICULO_ID?.value||'',CONDUCTOR_ID:form.elements.CONDUCTOR_ID?.value||'',NOTA_CONDUCTOR:form.querySelector('[data-ia-note]')?.value||'',LISTA_CODIFICADA:JSON.stringify(controls(form)),EVIDENCIAS:photos,ODOMETRO_INTENTOS:intentoActual});}
        catch(error){if(isAiUnavailable(error)){activateAiContingency(form,photos,error);return;}throw error;}
      ...
      const kmValue=Number(form.elements.KILOMETRAJE?.value||0);if(!(kmValue>0))throw new Error('Ingrese o confirme el kilometraje real visible en la fotografía del odómetro.');const kmManual=markKmContingencyIfChanged(form)||form.dataset.iaContingency==='1';if(form.dataset.iaContingency==='1'){const manualFlag=form.querySelector('[data-ia-km-manual-flag]');if(manualFlag)manualFlag.value='SI';}const kmConfirm=form.querySelector('[data-ia-km-confirm]');if(!kmConfirm||!kmConfirm.checked)throw new Error(`Confirme el kilometraje detectado (${Math.round(kmValue).toLocaleString('es-CL')} km) comparándolo con la fotografía del odómetro antes de guardar.`);const kmConfirmedValue=form.querySelector('[data-ia-km-confirm-value]');if(kmConfirmedValue)kmConfirmedValue.value=String(kmValue);
    ...
    }catch(error){if(isAiUnavailable(error)){const photos=safeJson(form.elements.FOTOS_VEHICULO?.value||'',[]);activateAiContingency(form,photos,error);}else{setStatus(form,'error','No se completó el proceso',String(error?.message||error));toast('Checklist IA',String(error?.message||error),'error');}}
    finally{if(submitter&&!handingOff){submitter.disabled=false;submitter.textContent=form.dataset.iaContingency==='1'?'✓ Confirmar Check-in sin IA':(analysedNow?'✓ Revisar 18 puntos y confirmar':(form.elements.SESION_IA_ID?.value?'✓ Confirmar Check-in':original));}}
  }
